...
/* Database connection start */
	...
	$file_mimes = array(
		...
		if(is_uploaded_file($_FILES['file']['tmp_name'])){
			...
			$ultimoMovimiento = $mysql->query('SELECT * FROM banco0840 ORDER BY id DESC LIMIT 1');
			...
			*/
			...
			// get data records from csv file
				...
				$sql_query = "SELECT id, fecha, descripcion, cargo, abono, saldo FROM banco0840 WHERE iden = '".$emp_record[6]."'";
				$resultset = mysqli_query($conn, $sql_query) or die("database error:". mysqli_error($conn));
					...
					if(mysqli_num_rows($resultset)) {
					 ...
					 $consulta = mysqli_query($conn,'SELECT MAX(numeroMovimiento) as numeroMovimiento FROM banco0840 LIMIT 1');
	...
	header("Location: banco0840".$import_status);
?>