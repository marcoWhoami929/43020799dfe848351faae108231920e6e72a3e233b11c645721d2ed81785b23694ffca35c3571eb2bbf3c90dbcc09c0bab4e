	...
	*/
	...
	static public function mdlMostrarLogistica($tabla, $item, $valor, $idUsuario){
		...
		if($item != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT  log.*,op.nombre as nombreOperador FROM $tabla as log LEFT OUTER JOIN operadores as op ON op.id = log.operador ORDER BY idPedido asc");
