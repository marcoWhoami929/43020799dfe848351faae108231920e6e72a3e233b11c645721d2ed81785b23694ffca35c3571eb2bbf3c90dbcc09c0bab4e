<?php
	...
	$file_mimes = array(
		...
		if(is_uploaded_file($_FILES['file']['tmp_name'])){
			...
			$ultimoMovimiento = $mysql->query('SELECT * FROM banco3450 ORDER BY id DESC LIMIT 1');
			...
			*/
			...
			// get data records from csv file
				...
				$sql_query = "SELECT id, fecha, descripcion, cargo, abono, saldo FROM banco3450 WHERE iden = '".$emp_record[6]."'";
					...
					if(mysqli_num_rows($resultset)) {
						...
						if ($cuotaTransaccion == true || $comisionVentasDebito == true || $comisionVentasCredito == true) {
							  ...
							  $mes=strftime("%B",mktime(0, 0, 0, $numero, 1, 2000)); 
							 	...
							 	$sql_update = "UPDATE banco3450 set mes = '".strtoupper($mes)."', fecha='".$emp_record[0]."', descripcion='".$emp_record[1]."', cargo='".str_replace(',','',$emp_record[2])."', abono='".str_replace(',','',$emp_record[3])."', saldo = '".str_replace(',','',$emp_record[4])."', grupo = 'EGRESOS', subgrupo = '07. Gastos Financieros  Comisiones Bancarias', acreedor = 'BBVA BANCOMER SA', concepto = 'COMISIONES BANCARIAS' WHERE iden='".$emp_record[6]."'";
							...
							$sql_update2 = "UPDATE banco3450 set mes = '".strtoupper($mes)."', fecha='".$emp_record[0]."', descripcion='".$emp_record[1]."', cargo='".str_replace(',','',$emp_record[2])."', abono='".str_replace(',','',$emp_record[3])."', saldo = '".str_replace(',','',$emp_record[4])."', grupo = 'EGRESOS', subgrupo = 'I.V.A Acreditable', acreedor = 'BBVA BANCOMER SA', concepto = 'COMISIONES BANCARIAS' WHERE iden='".$emp_record[6]."'";
							...
							$sql_update3 = "UPDATE banco3450 set mes = '".strtoupper($mes)."', fecha='".$emp_record[0]."', descripcion='".$emp_record[1]."', cargo='".str_replace(',','',$emp_record[2])."', abono='".str_replace(',','',$emp_record[3])."', saldo = '".str_replace(',','',$emp_record[4])."' WHERE iden='".$emp_record[6]."'";
						...
						if ( $cuotaTransaccion == true || $comisionVentasDebito == true || $comisionVentasCredito == true) {
							 ...
							 $consulta = mysqli_query($conn,'SELECT MAX(numeroMovimiento) as numeroMovimiento FROM banco3450 LIMIT 1');
							 ...
							 $codigo = (empty($consulta['numeroMovimiento']) ? 25415 : $consulta['numeroMovimiento']+=1);
	...
	header("Location: banco3450".$import_status);
?>