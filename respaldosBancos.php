      ...
      case 'respaldo0198':
        ...
        break;
         case 'respaldo7338':
            ...
            define("DB_NAME", 'matriz');
            define("DB_HOST", 'localhost');
            define("BACKUP_DIR", 'Respaldo-Banco7338'); // Comment this line to use same script's directory ('.')
            define("TABLES", 'banco7338'); // Full backup
            //define("TABLES", 'table1, table2, table3'); // Partial backup
            define("CHARSET", 'utf8');
            define("GZIP_BACKUP_FILE", true); // Set to false if you want plain SQL backup files (not gzipped)
            define("BATCH_SIZE", 1000); // Batch size when selecting rows from database in order to not exhaust system memory
            ...
            class Backup_Database
            {
                ...
                public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8')
                {
                    ...
                    $this->conn            = $this->initializeDatabase();
                    $this->backupDir       = BACKUP_DIR ? BACKUP_DIR : '.';
                    $this->backupFile      = 'banco7338-' . date("Ymd_His", time()) . '.sql';
                    $this->gzipBackupFile  = defined('GZIP_BACKUP_FILE') ? GZIP_BACKUP_FILE : true;
                    $this->batchSize       = defined('BATCH_SIZE') ? BATCH_SIZE : 1000; // default 1000 rows
                    $this->output          = '';
                }
                 ...
                 * @param string $tables
                 */
                public function backupTables($tables = 'banco7338')
                {
                    try {
                        /**
                         * Tables to export
