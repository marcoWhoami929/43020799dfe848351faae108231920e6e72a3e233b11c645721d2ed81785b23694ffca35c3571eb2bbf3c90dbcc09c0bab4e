...
/* Database connection start */
	...
	$file_mimes = array(
		...
		if(is_uploaded_file($_FILES['file']['tmp_name'])){
			...
			$ultimoMovimiento = $mysql->query('SELECT * FROM banco1286 ORDER BY id DESC LIMIT 1');
			...
			*/
			...
			// get data records from csv file
				...
				$sql_query = "SELECT id, fecha, descripcion, cargo, abono, saldo FROM banco1286 WHERE iden = '".$emp_record[6]."'";
				$resultset = mysqli_query($conn, $sql_query) or die("database error:". mysqli_error($conn));
					...
					if(mysqli_num_rows($resultset)) {
						...
						foreach ($resultset as $key => $value) {
							...
							$sql_update = "UPDATE banco1286 set mes = '".strtoupper($mes)."', fecha='".$emp_record[0]."', descripcion='".$emp_record[1]."', cargo='".str_replace(',','',$emp_record[2])."', abono='".str_replace(',','',$emp_record[3])."', saldo = '".str_replace(',','',$emp_record[4])."' WHERE iden='".$emp_record[6]."'";
					 ...
					 $consulta = mysqli_query($conn,'SELECT MAX(numeroMovimiento) as numeroMovimiento FROM banco1286 LIMIT 1');
	...
	header("Location: banco1286".$import_status);
?>