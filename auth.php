...
if(isset($_POST['signup']))
{
	...
	else
	{
		...
		$q=mysqli_query($conn,"insert into `user` (`fechaRegistro`,`nombreCompleto`,`email`,`password`, `taller`, `telefono`, `celular`, `direccion`, `latitud`, `longitud`) values ('$date','$nombreCompleto','$email','$password','$taller','$telefono','$celular', '$direccion', '$latitud', '$longitud')");
		if($q)
		{
			echo "success";
		}
...
if(isset($_POST['solicitudProceso']))
{
	...
	$solicitudEnviada = 1;
		...
		if (mysqli_num_rows($obtenerLista) == 4) {
			...
			$q=mysqli_query($conn,"insert into `solicitudes` (`cliente`,`idCliente`,`sucursal`,`observaciones`,`solicitudEnviada`,`listaProductos`) values ('$nombreCliente','$idCliente','$sucursalElegida','$observacionesSolicitud','$solicitudEnviada','$listaProductos')");
		...
		$listaNotificaciones = mysqli_query($conn,"Select max(nuevasSolicitudes) as total from notificacionesapp");
		while ($dato = mysqli_fetch_array($listaNotificaciones))
		{
			$valor = $dato["total"];
			if ($valor == 0) {

				$nuevaNotificacion = mysqli_query($conn, "UPDATE notificacionesapp SET nuevasSolicitudes = '$valor' + 1");
...
if(isset($_POST['login']))
{
	$email=mysqli_real_escape_string($conn,htmlspecialchars(trim($_POST['email'])));
	$password=mysqli_real_escape_string($conn,htmlspecialchars(trim($_POST['password'])));
	$datos = mysqli_query($conn,"select * from `user` where `email`='$email' and `password`='$password'");
	$login=mysqli_num_rows(mysqli_query($conn,"select * from `user` where `email`='$email' and `password`='$password'"));
	if($login!=0)
	{
...
if(isset($_POST['change_password']))
{
	...
	$check=mysqli_num_rows(mysqli_query($conn,"select * from `user` where `email`='$email' and `password`='$old_password'"));
	if($check!=0)
	{
		mysqli_query($conn,"update `user` set `password`='$new_password' where `email`='$email'");
		echo "success";
	}
	else
	{
...
if(isset($_POST['listarSolicitudes']))
{
	...
	$listarSolicitudes=mysqli_query($conn,"select id,sucursal,horaSolicitud,motoEnCamino,enProceso,motoEnCaminoRegreso,concluido,observaciones,listaProductos,rutaSolicitud from `solicitudes` where `idCliente`='$idCliente' ORDER BY horaSolicitud desc");
...
if(isset($_POST['listarFavoritos']))
{
	...
	$listarSolicitudes=mysqli_query($conn,"SELECT p.id, f.idProducto, f.id as idFavorito,f.codigoProducto,f.precioProducto,f.unidadElegida,pr.descripcion FROM productoscatalogo p, favoritos f, productos pr WHERE p.id = f.idProducto and  `idCliente`= '$idCliente' and f.codigoProducto = SUBSTRING_INDEX(pr.codigo, '.', 1) GROUP by idProducto");
...
if(isset($_POST['deleteFavoritos']))
{
	...
	$idCliente=mysqli_real_escape_string($conn,htmlspecialchars(trim($_POST['idCliente'])));
	    ...
	    $q=mysqli_query($conn,"DELETE from favoritos where idProducto = '$idProducto' and idCliente = '$idCliente'");
...
if(isset($_POST['forget_password']))
{
	...
	if($check!=0)
	{
		...
		$string="Hola ".$data['nombreCompleto']."  Su contraseña es " .$data['password'];
		...
		date_default_timezone_set('Etc/UTC');
		    ...
		    //CONFIGURACIÓN DEL EMISOR
		        ...
		        $mail->setFrom('cotizador.sfdk@gmail.com', 'SAN FRANCISCO DEKKERLAB');
...
if(isset($_POST['formContact']))
{
	...
	if(isset($_POST['formContact']))
	{
		...
		$string="Hola ha recibido un mensaje: <br> Usuario:".$nombreApellidos." <br>Correo de Contacto: ".$emailContacto."<br>Mensaje:  " .$comentariosContacto;

	    set_time_limit(0);
	    ignore_user_abort(true);
	    /*RECOGER VALORES ENVIADOS DESDE INDEX.PHP*/
	    $email = "disprosoft@gmail.com";
	    $sDestino = $email;
	    $sAsunto = "Mensaje de contacto de DEKKERAPP";
	    $sMensaje = $string;
		date_default_timezone_set('Etc/UTC');
		    require 'extensiones/PHPMailer/PHPMailerAutoload.php';
		    /*CONFIGURACIÓN DE CLASE*/
