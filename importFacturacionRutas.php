<?php
                    ...
                    if ($serie == "OTRT") {
                        ...
                        if(mysqli_num_rows($resultset)) {
                            ...
                            if(mysqli_num_rows($resultado)){
                                ...
                                $actualizacionFactura = "UPDATE facturasgenerales SET concepto = '".$concepto."', importeFactura = '".str_replace(',','',$emp_record[2])."',numeroPartidas = '".$emp_record[3]."',numeroUnidades = '".$emp_record[4]."',unidadesPendientes = '".$emp_record[4]."',pendiente = '".str_replace(',','',$emp_record[5])."',fechaFactura = '".$fechaFactura."', fechaVencimiento = '".$fechaVencimiento."',fechaCobro = '".$fechaCobro."',codigoCliente = '".$codigoCliente."',rfc = '".$rfc."',statusCliente = '".$statusCliente."',diasCredito = '".$diasCredito."',nombreCliente = '".$nameClient."', neto = '".number_format($neto,4,'.', '')."', impuesto = '".number_format($impuesto,4,'.', '')."', total ='".number_format($total,4,'.', '')."', estatus = '".$estatus."', formaPago = '".$formaPago."', agente = '".$agente."' where serie = '".$serieFactura."' and folio = '".$folioFactura."' and folioPedido = '".$folio."'";
                                mysqli_query($conn,$actualizacionFactura) or die("database error:".mysqli_error($conn));
                                ...
                                $sql_update = "INSERT INTO facturasgenerales(seriePedido,folioPedido,concepto,serie,folio,importeFactura,estatusFactura,numeroPartidas,numeroUnidades,unidadesPendientes,pendiente,fechaFactura,fechaVencimiento,codigoCliente,rfc,statusCliente,diasCredito,nombreCliente,numFactura,neto,impuesto,total,estatus,formaPago, agente) VALUES('".$serie."','".$folio."','".$concepto."','".$emp_record[0]."','".str_replace(',','',$emp_record[1])."','".str_replace(',','',$emp_record[2])."',$estatusFactura,'".$emp_record[3]."','".$emp_record[4]."','".$emp_record[4]."','".str_replace(',','',$emp_record[5])."','".$fechaFactura."','".$fechaVencimiento."','".$codigoCliente."','".$rfc."','".$statusCliente."','".$diasCredito."','".$nameClient."','".$numeroFactura."','".number_format($neto,4,'.', '')."','".number_format($impuesto,4,'.', '')."','".number_format($total,4,'.', '')."','".$estatus."','".$formaPago."','".$agente."')";
                                mysqli_query($conn, $sql_update) or die("database error:". mysqli_error($conn));
                                ...
                                $obtenerDatosFactura = "SELECT COUNT(id) as secciones,SUM(importeFactura) as importeSurtido, SUM(numeroUnidades) as unidadesSurtidas from facturasgenerales where folioPedido = '".$folio."'";
                                $respuesta = mysqli_query($conn,$obtenerDatosFactura ) or die("database error:".mysqli_error($conn));
                                $datos = mysqli_fetch_array($respuesta);
    ...
    header("Location: facturacionRuta".$import_status);
?>