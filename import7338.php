...
if (isset($_POST['import_data'])) {
    ...
    if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        if (is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			$ultimoMovimiento = $mysql->query('SELECT * FROM banco7338 ORDER BY id DESC LIMIT 1');
			...
			*/
            ...
            while (($emp_record = fgetcsv($csv_file, 1000, ",")) !== FALSE) {
                ...
                $sql_query = "SELECT id, fecha, descripcion, cargo, abono, saldo FROM banco7338 WHERE iden = '" . $emp_record[6] . "'";
                ...
                if (mysqli_num_rows($resultset)) {
                    ...
                    foreach ($resultset as $key => $value) {
                        ...
                        $sql_update = "UPDATE banco7338 set mes = '" . strtoupper($mes) . "', fecha='" . $emp_record[0] . "', descripcion='" . $emp_record[1] . "', cargo='" . str_replace(',', '', $emp_record[2]) . "', abono='" . str_replace(',', '', $emp_record[3]) . "', saldo = '" . str_replace(',', '', $emp_record[4]) . "' WHERE iden='" . $emp_record[6] . "'";
